...
import {
  useGetAllLanguages,
  useGetWordsByDifficulty,
  useGetWordsCountForLanguage,
...
export default function LanguagePage() {
  ...
  const [selectedDifficulty, setSelectedDifficulty] =
    useState<DifficultySelector>(DifficultySelector.all);
  ...
  const { data: _totalCount } = useGetWordsCountForLanguage(languageName);
  ...
  const {
    ...
    isError,
    error,
  } = useGetWordsByDifficulty(languageName, selectedDifficulty);
  ...
  const displayWords = filteredWords?.slice(
  ...
  const handleGameStart = (gameId: GameMode) => {
    if (!fetchedWords || fetchedWords.length === 0) {
      toast.error(
        "No vocabulary available for this language yet. Please add words in the admin panel.",
      );
      return;
    }
    ...
    return (
      <div className="flex items-center justify-center min-h-screen p-4">
        <Alert className="max-w-md">
          ...
          <AlertDescription>
            Language "{languageName}" not found. Please return to the home page
            and select a valid language.
            <br />
            <Button
              onClick={() => navigate({ to: "/" })}
              className="mt-4"
              ...
              return (
                ...
                >
                  ...
                  <div className="glass-card p-6 h-full min-h-[180px] flex flex-col">
                    ...
                    <div
                      className={`absolute inset-0 bg-gradient-to-br ${game.bgGradient} opacity-0 group-hover:opacity-100 transition-opacity duration-300`}
                    />
                    ...
                    <div className="relative z-10 flex flex-col h-full">
                      {/* Icon with Gradient */}
                      <div
                        className={`w-14 h-14 rounded-lg bg-gradient-to-br ${game.gradient} flex items-center justify-center mb-4 shadow-lg group-hover:scale-110 transition-transform duration-300`}
                      >
            ...
            <AlertDescription className="text-amber-800 dark:text-amber-200">
              {fetchedWords && fetchedWords.length === 0 ? (
                <>
                  ...
                  <br />
                  An administrator needs to add English vocabulary words for
                  this language. Please contact your administrator or visit the
