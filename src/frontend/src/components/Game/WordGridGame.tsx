...
export default function WordGridGame({
  ...
  const getCategoryDescription = (cat: Category): string => {
    switch (cat) {
      case "short":
        return "Short words (5 letters or less)";
      case "long":
        return "Long words (more than 8 letters)";
      case "beginner":
        return "Beginner level words";
      case "advanced":
        return "Advanced level words";
    }
  };
  ...
  const matchesCategory = (word: Word, cat: Category): boolean => {
    switch (cat) {
      case "short":
        return word.english.length <= 5;
      case "long":
        return word.english.length > 8;
      case "beginner":
        return word.difficulty === Difficulty.beginner;
      case "advanced":
        return word.difficulty === Difficulty.advanced;
    }
  };
  ...
  useEffect(() => {
    ...
    const nonMatching = filteredWords.filter(
      (w) => !matchesCategory(w, category),
    );

    const selectedMatching = matching
      .sort(() => Math.random() - 0.5)
      .slice(0, 4);
    const selectedNonMatching = nonMatching
      .sort(() => Math.random() - 0.5)
      .slice(0, 8);

    const grid = [...selectedMatching, ...selectedNonMatching].sort(
      () => Math.random() - 0.5,
    );
    setGridWords(grid);
  ...
  const handleSubmit = () => {
    ...
    const correctSelections = Array.from(selectedWords).filter((w) =>
      correctWords.has(w),
    ).length;
          ...
          <CardContent className="pt-6">
            <div className="text-center py-8 space-y-4">
              <p className="text-muted-foreground">
                Not enough words for the selected difficulty. At least 12 words
                are required.
              </p>
              <DifficultySelector
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Select all words matching the category
          </p>
        </div>
      </div>
