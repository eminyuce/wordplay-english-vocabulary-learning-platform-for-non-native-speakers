import { ArrowLeft, Trophy, Wrench } from "lucide-react";
...
export default function FixTheWordGame({
  words,
  language,
  onClose,
}: FixTheWordGameProps) {
  ...
  const generateMisspelling = useCallback((word: string): string => {
  ...
  useEffect(() => {
    ...
    setUserInput("");
    setIsAnswered(false);
  }, [currentWord, generateMisspelling]);
  ...
  const handleSubmit = useCallback(() => {
    ...
    if (correct) {
      ...
      toast.error(`Wrong! The correct spelling is: ${currentWord.english}`, {
        duration: 2000,
      });
    }
