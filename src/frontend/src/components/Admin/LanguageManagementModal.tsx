...
export default function LanguageManagementModal({
  ...
  const [editingLanguage, setEditingLanguage] =
    useState<EditingLanguage | null>(null);
  ...
  const handleRemoveLanguage = async (languageName: string) => {
    if (!isAdminAuthenticated) {
      setValidationErrors({
        general:
          "You must be logged in as admin to remove languages. Please log in via the Admin Login page.",
      });
      return;
    }
  ...
  const validateLanguageEdit = (editing: EditingLanguage): boolean => {
    ...
    if (!editing.name.trim()) {
      ...
      editing.name !== editing.originalName &&
      languages?.some((l) => l.name === editing.name)
    ) {
    ...
    if (Number.isNaN(orderingNum) || orderingNum < 1) {
  ...
  const handleSaveLanguage = async () => {
    ...
    if (!isAdminAuthenticated) {
      setValidationErrors({
        general:
          "You must be logged in as admin to update languages. Please log in via the Admin Login page.",
      });
      return;
    }
  ...
  const handleFieldChange = (
    field: keyof EditingLanguage,
    value: string | TextDirection,
  ) => {
    if (!editingLanguage) return;
    setEditingLanguage({
      ...editingLanguage,
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="glass-modal max-w-3xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-2xl glass-text">
            Manage Languages
          </DialogTitle>
          <DialogDescription className="glass-text-muted">
            Edit language properties or remove languages. Removing a language
            will also delete all associated words.
          </DialogDescription>
        </DialogHeader>
        ...
        {!isAdminAuthenticated && (
          <Alert className="glass-alert bg-yellow-50 dark:bg-yellow-950 border-yellow-200 dark:border-yellow-800">
            ...
            <AlertDescription className="text-yellow-800 dark:text-yellow-200">
              You must be logged in as admin to manage languages. Please log in
              via the Admin Login page.
            </AlertDescription>
          </Alert>
        )}
          ...
          {isLoading ? (
            <div className="space-y-3">
              {[...Array(3)].map((_, i) => {
                ...
                return <Skeleton key={`skel-${i}`} className="h-20 w-full" />;
              })}
            </div>
          ) : !languages || languages.length === 0 ? (
            <p className="text-center glass-text-muted py-8">
                ...
                {editingLanguage?.originalName === language.name ? (
                  ...
                  <div className="p-6 border rounded-lg glass-card-inner space-y-4">
                    <div className="flex items-center justify-between mb-4">
                      <h4 className="font-semibold glass-text text-lg">
                        Edit Language
                      </h4>
                      <Button
                        size="sm"
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      <div className="space-y-2">
                        ...
                        <Input
                          ...
                          onChange={(e) =>
                            handleFieldChange("flagEmoji", e.target.value)
                          }
                          disabled={isUpdating}
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="flex gap-2">
                          <Input
                            ...
                            onChange={(e) =>
                              handleFieldChange("gradientStart", e.target.value)
                            }
                            disabled={isUpdating}
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="flex gap-2">
                          <Input
                            ...
                            onChange={(e) =>
                              handleFieldChange("gradientEnd", e.target.value)
                            }
                            disabled={isUpdating}
                            className="w-16 h-10 p-1 glass-input"
                          />
                      ...
                      <div className="space-y-2">
                        ...
                        <Input
                          ...
                          onChange={(e) =>
                            handleFieldChange("ordering", e.target.value)
                          }
                          disabled={isUpdating}
                  ...
                  <div className="flex items-center gap-4 p-4 border rounded-lg glass-card-inner transition-all hover:shadow-md">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-sm glass-text-muted">
                        ...
                        {language.textDirection === "rtl" ? "RTL" : "LTR"} •
                        Order: {String(language.ordering)}
                      </p>
                    </div>
                        ...
                        <AlertDialogContent className="glass-modal">
                          <AlertDialogHeader>
                            ...
                            <AlertDialogDescription className="glass-text-muted">
                              ...
                              {language.name}" and all associated words. This
