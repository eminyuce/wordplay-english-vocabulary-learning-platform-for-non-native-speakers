...
function convertDifficultyFilterToBackend(
...
export default function AdminWordManagement() {
  ...
  const [difficultyFilter, setDifficultyFilter] =
    useState<DifficultyFilter>("Beginner");
  ...
  const [isBulkLanguageImportModalOpen, setIsBulkLanguageImportModalOpen] =
    useState(false);
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const difficultySelector =
        convertDifficultyFilterToBackend(difficultyFilter);
  ...
  const totalLanguagePages = Math.ceil(
    sortedLanguages.length / languagePageSize,
  );
  ...
  const handleDeleteLanguage = async (languageName: string) => {
    ...
    try {
      ...
      if (
        languageCurrentPage >
        Math.ceil((sortedLanguages.length - 1) / languagePageSize)
      ) {
  ...
  const handleExportCSV = async () => {
    ...
    try {
      ...
      const formattedRows = csvRows.map((row) => {
        const [english, foreign, languageName, difficulty, examples, addedAt] =
          row;
        return [
          `"${english.replace(/"/g, '""')}"`,
          `"${foreign.replace(/"/g, '""')}"`,
  ...
  }, [selectedLanguage, difficultyFilter]);
        ...
        >
          <TabsList className="glass-card p-1 grid w-full grid-cols-4 gap-1">
            <TabsTrigger
              value="vocabulary"
              className={
                activeTab === "vocabulary" ? "admin-tab-active" : "glass-tab"
              }
            >
              Vocabulary
            </TabsTrigger>
            <TabsTrigger
              value="languages"
              className={
                activeTab === "languages" ? "admin-tab-active" : "glass-tab"
              }
            >
              Languages
            </TabsTrigger>
            <TabsTrigger
              value="quotes"
              className={
                activeTab === "quotes" ? "admin-tab-active" : "glass-tab"
              }
            >
              Quotes
            </TabsTrigger>
            <TabsTrigger
              value="feedback"
              className={
                activeTab === "feedback" ? "admin-tab-active" : "glass-tab"
              }
            >
              Feedback
            </TabsTrigger>
                ...
                <div className="flex flex-wrap gap-2">
                  {(
                    ...
                    <Button
                      ...
                      variant={
                        difficultyFilter === diff ? "default" : "outline"
                      }
                      className={
            ...
            <div className="glass-card rounded-2xl p-6 shadow-lg">
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-4">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Select
                    value={String(vocabPageSize)}
                    onValueChange={handleVocabPageSizeChange}
                  >
                                  ...
                                  <AlertDialogContent className="glass-modal">
                                    <AlertDialogHeader>
                                      ...
                                      <AlertDialogDescription className="glass-text-muted">
                                        ...
                                        {word.english}"? This action cannot be
                                        undone.
                                      </AlertDialogDescription>
                                    </AlertDialogHeader>
                                    <AlertDialogFooter>
          ...
          <TabsContent value="languages">
            <div className="glass-card rounded-2xl p-6 shadow-lg">
              ...
              <div className="flex items-center gap-2 mb-4">
                <Label className="text-sm font-medium">Page Size:</Label>
                <Select
                  value={String(languagePageSize)}
                  onValueChange={handleLanguagePageSizeChange}
                >
                  <SelectTrigger className="w-24 glass-input">
                    <SelectValue />
                  </SelectTrigger>
                              ...
                              <AlertDialogContent className="glass-modal">
                                <AlertDialogHeader>
                                  ...
                                  <AlertDialogDescription className="glass-text-muted">
                                    ...
                                    {lang.name}" and all associated words. This
                                ...
                                <AlertDialogFooter>
                                  ...
                                  <AlertDialogAction
                                    onClick={() =>
                                      handleDeleteLanguage(lang.name)
                                    }
                                    className="bg-destructive text-destructive-foreground hover:bg-destructive/90"
                                  >
                                    Remove Language
              ...
              {totalLanguagePages > 1 && (
                <div className="flex items-center justify-between mt-4 pt-4 border-t border-gray-200 dark:border-gray-700">
                  <div className="text-sm text-gray-600 dark:text-gray-400">
                    Showing {(languageCurrentPage - 1) * languagePageSize + 1}{" "}
                    to{" "}
                    {Math.min(
                      languageCurrentPage * languagePageSize,
                      sortedLanguages.length,
                    )}{" "}
                    of {sortedLanguages.length} languages
                  </div>
                  <div className="flex items-center gap-2">
                    <Button
