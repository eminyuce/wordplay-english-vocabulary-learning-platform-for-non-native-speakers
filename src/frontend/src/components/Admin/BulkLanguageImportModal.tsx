...
export default function BulkLanguageImportModal({
  open,
  onClose,
}: BulkLanguageImportModalProps) {
  ...
  const handleImport = async () => {
    ...
    for (let i = 0; i < dataLines.length; i++) {
      ...
      if (fields.length < 7) {
        errors.push(
          `Line ${i + 1}: Insufficient fields (expected 7, got ${fields.length})`,
        );
        continue;
      }

      const [name, code, flag, textDir, startColor, endColor, orderingStr] =
        fields;

      // Validate required fields
      if (!name || !code || !flag) {
        errors.push(
          `Line ${i + 1}: Missing required fields (name, code, or flag)`,
        );
        continue;
      }
      ...
      const direction =
        textDir.toLowerCase() === "rtl" ? TextDirection.rtl : TextDirection.ltr;

      // Validate ordering
      const ordering = Number.parseInt(orderingStr, 10);
      if (Number.isNaN(ordering) || ordering < 1) {
      ...
      <DialogContent className="glass-modal max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-2xl glass-text">
            Bulk Import Languages (CSV)
          </DialogTitle>
          <DialogDescription className="glass-text-muted">
            Import multiple languages from CSV. Expected format:
            name,code,flag,textDirection,startColor,endColor,ordering
          </DialogDescription>
        </DialogHeader>
        ...
        >
          <TabsList className="glass-card p-1 grid w-full grid-cols-2 gap-1">
            <TabsTrigger
              value="paste"
              className={
                inputMode === "paste" ? "admin-tab-active" : "glass-tab"
              }
            >
            ...
            <TabsTrigger
              value="upload"
              className={
                inputMode === "upload" ? "admin-tab-active" : "glass-tab"
              }
            >
          ...
          <TabsContent value="paste" className="space-y-4 mt-4">
            <div className="space-y-2">
              ...
              <p className="text-xs glass-text-muted">
                Each line:
                name,code,flag,textDirection(ltr/rtl),startColor,endColor,ordering
              </p>
            </div>
          </TabsContent>
