...
export default function AdminPanel() {
  ...
  const handleSeedLanguages = async () => {
    try {
      ...
      toast.success(
        "Initial languages seeded successfully! 6 languages have been created.",
      );
    } catch (error: any) {
      ...
      <Card className="mt-8 border-2 border-primary/20 bg-gradient-to-br from-purple-50 to-pink-50 dark:from-purple-900/20 dark:to-pink-900/20">
        ...
        <CardContent className="space-y-4">
          <p className="text-sm text-muted-foreground">
            Manage target languages and English vocabulary content for the
            learning platform.
          </p>
          ...
          {hasNoLanguages && (
            <Alert className="bg-amber-50 dark:bg-amber-900/20 border-amber-200 dark:border-amber-800">
              <AlertDescription className="text-amber-800 dark:text-amber-200">
                <strong>No languages found!</strong> Click "Seed Initial
                Languages" below to create 6 starter languages (Turkish,
                Spanish, Arabic, German, Japanese, French), or create languages
                manually.
              </AlertDescription>
            </Alert>
          )}
          ...
          <div className="space-y-2">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <Button
                onClick={() => setShowImportVocab(true)}
                variant="outline"
                className="gap-2"
              >
