...
export default function CsvImportModal({ open, onClose }: CsvImportModalProps) {
  ...
  const validateRow = (row: CsvRow, rowNumber: number): ValidationResult => {
    ...
    if (!row.Level || row.Level.trim() === "") {
      ...
      if (!matchedLevel) {
        errors.push(
          `Invalid Level value "${row.Level}". Must be one of: Beginner, Medium, Hard, Advanced`,
        );
      }
    }
    ...
    if (!row.Example_Sentence || row.Example_Sentence.trim() === "") {
      ...
      if (!exampleSentence.includes(englishWord)) {
        errors.push(
          `Example_Sentence must contain the English_Word "${row.English_Word}" (case-insensitive)`,
        );
      }
    }
  ...
  const parseCSV = (text: string): CsvRow[] => {
    ...
    if (header.length !== expectedHeaders.length) {
      throw new Error(
        `Invalid CSV format. Expected ${expectedHeaders.length} columns: ${expectedHeaders.join(", ")}`,
      );
    }
    ...
    for (let i = 0; i < expectedHeaders.length; i++) {
      if (header[i] !== expectedHeaders[i]) {
        throw new Error(
          `Invalid header at column ${i + 1}. Expected "${expectedHeaders[i]}", got "${header[i]}"`,
        );
      }
    }
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      if (values.length !== expectedHeaders.length) {
        throw new Error(
          `Row ${i + 1}: Expected ${expectedHeaders.length} columns, got ${values.length}`,
        );
      }
  ...
  const processCSVContent = (text: string) => {
    ...
    try {
      ...
      const results: ValidationResult[] = rows.map(
        (row, index) => validateRow(row, index + 2), // +2 because row 1 is header, data starts at row 2
      );
      ...
      if (validRows.length === 0) {
        setValidationError(
          `All ${rows.length} rows are invalid. Please fix the errors and try again.`,
        );
        return;
      }
      ...
      <DialogContent className="glass-modal max-w-3xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription className="glass-text-muted">
            Upload a CSV file or paste CSV content directly. English word is the
            unique primary key. Duplicate words and duplicate example sentences
              ...
              <Alert className="glass-alert">
                ...
                <AlertDescription>
                  ...
                  <div className="mt-2 space-y-1 text-sm">
                    ...
                    <div>
                      ❌ <strong>Invalid rows:</strong> {invalidResults.length}
                    </div>
                    <div>
                      📦 <strong>Unique words to import:</strong>{" "}
                      {validEntries.length}
                    </div>
                  </div>
                  <div className="mt-2 text-xs glass-text-muted">
                    Only valid rows will be imported. Duplicate English words
                    and duplicate example sentences are skipped automatically.
                  </div>
                </AlertDescription>
              </Alert>
              ...
              {invalidResults.length > 0 && (
                <details className="glass-card-inner rounded-lg p-3">
                  <summary className="cursor-pointer font-medium text-sm flex items-center gap-2 glass-text">
                    ...
                    View {invalidResults.length} Invalid Row
                    {invalidResults.length !== 1 ? "s" : ""} with Errors
                  </summary>
                        ...
                        >
                          ...
                          <ul className="mt-1 space-y-0.5">
                            {result.errors.map((error, idx) => (
                              <li
                                key={`error-${result.rowNumber}-${idx}`}
              ...
              <Alert className="glass-alert">
                ...
                <AlertDescription>
                  ...
                  <div className="mt-2 space-y-1 text-sm">
                    ...
                    <div>
                      ✅ <strong>Valid rows:</strong> {importSummary.validRows}
                    </div>
                    <div>
              ...
              {importSummary.rowErrors.length > 0 && (
                <details className="glass-card-inner rounded-lg p-3">
                  <summary className="cursor-pointer font-medium text-sm flex items-center gap-2 glass-text">
                    ...
                    View Per-Row Error Details ({importSummary.rowErrors.length}{" "}
                    rows)
                  </summary>
                  <ScrollArea className="h-48 mt-3">
                    <div className="space-y-3 pr-4">
                        ...
                        >
                          ...
                          <ul className="mt-1 space-y-0.5">
                            {rowError.errors.map((error, idx) => (
                              <li
                                key={`row-error-${rowError.rowNumber}-${idx}`}
...
book, kitap, Beginner, Turkish, I love reading books.`}
              ...
              <div className="space-y-2 text-xs glass-text-muted">
                <p>
                  <strong>Strict Validation Rules (per row):</strong>
                </p>
                <ul className="list-disc list-inside space-y-1 ml-2">
                  <li>
                    <strong>English_Word:</strong> Text, non-empty (required)
                  </li>
                  <li>
                    <strong>Translation:</strong> Text, non-empty (required)
                  </li>
                  <li>
                    <strong>Level:</strong> Must be one of: Beginner, Medium,
                    Hard, Advanced (case-insensitive, required)
                  </li>
                  <li>
                    <strong>Target_Language:</strong> Text, non-empty (required)
                  </li>
                  <li>
                    <strong>Example_Sentence:</strong> Text, non-empty, must
                    contain the English_Word case-insensitively (required)
                  </li>
                </ul>
                <p className="mt-2">
                ...
                <ul className="list-disc list-inside space-y-1 ml-2">
                  ...
                  <li>
                    Invalid rows are skipped entirely without failing the import
                  </li>
                  <li>
                    Valid rows with same English_Word + Target_Language are
                    grouped together
                  </li>
                  <li>
                    Up to 5 example sentences per word (additional rows ignored)
                  </li>
