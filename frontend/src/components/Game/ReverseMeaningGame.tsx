...
import { Word, Difficulty, DifficultySelector as DifficultyType, Language } from '../../backend';
...
interface ReverseMeaningGameProps {
  words: Word[];
  language: Language;
  languageName: string;
  onClose: () => void;
...
const ReverseMeaningGame: React.FC<ReverseMeaningGameProps> = ({ words: allWords, language, languageName, onClose }) => {
  const [selectedDifficulty, setSelectedDifficulty] = useState<DifficultyType>('all' as DifficultyType);
  ...
  const { currentMessage, showOverlay, checkProgress, resetProgress, dismissOverlay } = useSessionProgress(10);

  const { checkReward, showReward, rewardQuote, rewardSuccessRate, closeReward } = useGameReward();
    ...
    if (allWords.length > 0) {
      const filtered =
        selectedDifficulty === 'all'
          ? allWords
          : allWords.filter((w) => w.difficulty === (selectedDifficulty as unknown as Difficulty));
  ...
  const generateOptions = () => {
    ...
    const otherWords = gameWords.filter((_, idx) => idx !== currentQuestionIndex);
      ...
      <div className="max-w-2xl mx-auto">
        ...
        <div className="bg-white rounded-lg shadow-md p-4 mb-4">
          <div className="flex items-center gap-3 mb-3">
            <button
              onClick={handleBackClick}
              className="flex items-center gap-2 px-4 py-2 rounded-lg bg-gradient-to-r from-purple-50 to-pink-50 hover:from-purple-100 hover:to-pink-100 text-purple-700 font-medium transition-all duration-200 hover:shadow-md backdrop-blur-sm border border-purple-200"
            ...
            <h1 className="text-xl font-bold text-gray-800">Reverse Meaning</h1>
            <button
              onClick={handleBackClick}
          ...
          <div className="mt-3 px-4 py-2 bg-gradient-to-r from-purple-50 to-pink-50 rounded-lg border border-purple-200">
        ...
        <div className="bg-white rounded-lg shadow-md p-6 mb-6">
          ...
          <p className="text-3xl font-bold text-purple-600 mb-6">{currentWord.foreign}</p>
        ...
        {selectedAnswer !== null && (
          ...
          >
            {currentQuestionIndex < gameWords.length - 1 ? 'Next Question' : 'Finish Game'}
          </button>
        )}
      </div>
...
export default ReverseMeaningGame;
