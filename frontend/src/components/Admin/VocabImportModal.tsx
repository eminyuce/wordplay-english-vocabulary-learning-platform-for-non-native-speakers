...
import { Upload, FileJson, AlertCircle, CheckCircle2, XCircle, ClipboardPaste } from 'lucide-react';
...
export default function VocabImportModal({ open, onClose }: VocabImportModalProps) {
  ...
  const validateEntry = (entry: any, index: number): { valid: boolean; errors: string[]; entry: Word | null } => {
    const errors: string[] = [];

    if (!entry.english || typeof entry.english !== 'string' || entry.english.trim() === '') {
      errors.push('Missing or invalid "english" field (must be non-empty string)');
    }

    if (!entry.foreign || typeof entry.foreign !== 'string' || entry.foreign.trim() === '') {
      errors.push('Missing or invalid "foreign" field (must be non-empty string)');
    }

    if (!entry.language || typeof entry.language !== 'string' || entry.language.trim() === '') {
      errors.push('Missing or invalid "language" field (must be non-empty string)');
    }
    ...
    if (!entry.difficulty || typeof entry.difficulty !== 'string') {
      ...
      if (!matchedDifficulty) {
        errors.push(`Invalid difficulty "${entry.difficulty}". Must be one of: Beginner, Medium, Hard, Advanced`);
      }
    }
    ...
    if (!Array.isArray(entry.examples)) {
      ...
      errors.push('Examples array cannot be empty (at least 1 example required)');
    } else if (entry.examples.length > 5) {
      errors.push('Maximum 5 examples allowed per word');
    } else {
      for (let i = 0; i < entry.examples.length; i++) {
        if (typeof entry.examples[i] !== 'string' || entry.examples[i].trim() === '') {
  ...
  const processJSONContent = (text: string) => {
    ...
    try {
      ...
      if (validatedEntries.length === 0) {
        setValidationError(`All ${data.length} entries are invalid. Please fix the errors and try again.`);
      ...
      <DialogContent className="glass-modal max-w-3xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription className="glass-text-muted">
            Upload a JSON file or paste JSON content directly. English word is the unique primary key. Duplicate words and duplicate example sentences are skipped automatically.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4">
          ...
          {(validEntries.length > 0 || validationErrors.length > 0) && !importSummary && (
            <div className="space-y-2">
              <Alert className="glass-alert">
                ...
                <AlertDescription>
                  ...
                  <div className="mt-2 space-y-1 text-sm">
                    <div>✅ <strong>Valid entries:</strong> {validEntries.length}</div>
                    <div>❌ <strong>Invalid entries:</strong> {validationErrors.length}</div>
                  </div>
                  <div className="mt-2 text-xs glass-text-muted">
                    Languages: {Array.from(new Set(validEntries.map(e => e.languageName))).join(', ') || 'None'}
                  </div>
                  <div className="mt-1 text-xs glass-text-muted">
                    Only valid entries will be imported. Duplicate English words and duplicate example sentences are skipped automatically.
                  </div>
                </AlertDescription>
              </Alert>
              ...
              {validationErrors.length > 0 && (
                <details className="glass-card-inner rounded-lg p-3">
                  <summary className="cursor-pointer font-medium text-sm flex items-center gap-2 glass-text">
                    ...
                    View {validationErrors.length} Invalid Entr{validationErrors.length !== 1 ? 'ies' : 'y'} with Errors
                  </summary>
                  <ScrollArea className="h-48 mt-3">
                    <div className="space-y-3 pr-4">
                      {validationErrors.map((error) => (
                        <div key={error.entryIndex} className="border-l-2 border-destructive pl-3 py-1">
                          <div className="font-medium text-sm glass-text">Entry {error.entryIndex}</div>
                          <div className="text-xs glass-text-muted mt-1">
                            English: {error.entry?.english || '(missing)'} | 
                            Language: {error.entry?.language || '(missing)'} | 
                            Difficulty: {error.entry?.difficulty || '(missing)'}
                          </div>
                          <ul className="mt-1 space-y-0.5">
                            {error.errors.map((err, idx) => (
                              <li key={idx} className="text-xs text-destructive">• {err}</li>
                            ))}
                          </ul>
                        </div>
                      ))}
            ...
            <div className="space-y-3">
              <Alert className="glass-alert">
                ...
                <AlertDescription>
                  ...
                  <div className="mt-2 space-y-1 text-sm">
                    <div>📊 <strong>Total entries processed:</strong> {importSummary.totalEntries}</div>
                    <div>✅ <strong>Valid entries:</strong> {importSummary.validEntries}</div>
                    <div>❌ <strong>Invalid entries skipped:</strong> {importSummary.invalidEntries}</div>
              ...
              {importSummary.entryErrors.length > 0 && (
                <details className="glass-card-inner rounded-lg p-3">
                  <summary className="cursor-pointer font-medium text-sm flex items-center gap-2 glass-text">
                    ...
                    View Per-Entry Error Details ({importSummary.entryErrors.length} entries)
                  </summary>
                  <ScrollArea className="h-48 mt-3">
                    <div className="space-y-3 pr-4">
                      {importSummary.entryErrors.map((error) => (
                        <div key={error.entryIndex} className="border-l-2 border-orange-600 pl-3 py-1">
                          <div className="font-medium text-sm glass-text">Entry {error.entryIndex}</div>
                          <ul className="mt-1 space-y-0.5">
                            {error.errors.map((err, idx) => (
                              <li key={idx} className="text-xs text-orange-600">• {err}</li>
                            ))}
                          </ul>
                        </div>
                      ))}
    ...
    "examples": [
      ...
      "The sunset was beautiful."
              ...
              <div className="space-y-2 text-xs glass-text-muted">
                <p><strong>Strict Validation Rules (per entry):</strong></p>
                <ul className="list-disc list-inside space-y-1 ml-2">
                  <li><strong>english:</strong> String, non-empty (required)</li>
                  <li><strong>foreign:</strong> String, non-empty (required)</li>
                  <li><strong>language:</strong> String, non-empty (required)</li>
                  <li><strong>difficulty:</strong> Must be one of: Beginner, Medium, Hard, Advanced (case-insensitive, required)</li>
                  <li><strong>examples:</strong> Array of strings, 1-5 examples, each non-empty (required)</li>
                </ul>
                ...
                <ul className="list-disc list-inside space-y-1 ml-2">
                  ...
                  <li>Invalid entries are skipped entirely without failing the import</li>
                  <li>Detailed error reasons provided for each invalid entry</li>
