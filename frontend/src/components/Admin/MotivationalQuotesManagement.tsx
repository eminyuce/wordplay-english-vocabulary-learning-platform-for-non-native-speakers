...
import { MotivationalQuote } from '../../backend';
...
export default function MotivationalQuotesManagement() {
  ...
  const { data: quotes = [], isLoading, refetch } = useGetAllMotivationalQuotes();
  ...
  return (
    <div className="glass-card rounded-2xl p-6 shadow-lg space-y-6">
      <div className="flex items-center justify-between">
        <h2 className="text-2xl font-bold glass-text">Motivational Quotes Management</h2>
        <Button
      ...
      {showBulkInput && (
        <div className="space-y-3">
          <Label className="text-sm font-medium">Bulk Add Quotes (One Per Line)</Label>
          <Textarea
                ...
                {quotes.map((quote, index) => (
                  <tr
                    ...
                    className={`glass-table-row ${
                      index % 2 === 0 ? 'glass-table-row-even' : 'glass-table-row-odd'
                    }`}
                  >
      ...
      {/* Edit Quote Dialog */}
      {editingQuote && (
        <AlertDialog open={!!editingQuote} onOpenChange={() => setEditingQuote(null)}>
          <AlertDialogContent className="glass-modal">
            <AlertDialogHeader>
              <AlertDialogTitle className="glass-text">Edit Quote</AlertDialogTitle>
            </AlertDialogHeader>
